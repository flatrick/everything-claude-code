...
A variable/value is **neutral** when it does not influence tool detection and cannot force `cursor` or `claude`.

Examples:

- Unrecognized vars (`NODE_ENV`, `CI`, etc.)
- Recognized vars with non-matching values (`CURSOR_AGENT="0"`, `CLAUDE_CODE="0"`)
