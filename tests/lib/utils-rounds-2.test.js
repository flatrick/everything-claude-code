...
function runTests() {
  ...
  if (test('grepFile finds Unicode emoji patterns across lines', () => {
    ...
    try {
      fs.writeFileSync(testFile, 'ð celebration\nnormal line\nð party\næ¥æ¬èªãã¹ã');
      const emojiResults = utils.grepFile(testFile, /ð/);
