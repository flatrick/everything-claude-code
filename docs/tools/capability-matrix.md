...
| Rules / reusable guidance | `official` markdown docs/rules; user-level rules in `~/.claude/rules/` are file-based | `official` project `.cursor/rules/`; `locally-verified` user-global `~/.cursor/rules/*.mdc` via `cursor-agent` (official docs still describe user rules differently, so this may be a CLI-vs-IDE distinction) | `official` `.rules` files under Codex config layers |
| Skills / reusable workflows | `official` Claude Code skills docs exist; MDT ships `skills/*/SKILL.md` | `official` `.cursor/skills/` (project) and `~/.cursor/skills/` (user); same `SKILL.md` format; MDT ships `frontend-slides`, others need adding | `official` skills under project `.codex/skills` or user/system skill locations; MDT now installs package-selected Codex skills from `codex-template/skills/` |
...
| Persistent context / memory | `official` `CLAUDE.md` and memory docs | `official` rules, memories, and `AGENTS.md` | `official` persistent layered instructions; `experimental`, `locally-verified` local `memories` feature flag exists; MDT also uses project-local `.codex/` install and runtime state for explicit continuous-learning |
