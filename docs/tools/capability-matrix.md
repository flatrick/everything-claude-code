...
| Rules / reusable guidance | `official` markdown docs/rules; user-level rules in `~/.claude/rules/` are file-based | `official` `.cursor/rules/` at project scope only; user-level rules are database-backed and not file-installable | `official` `.rules` files under Codex config layers |
